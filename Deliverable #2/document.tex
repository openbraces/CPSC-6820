...
\subsubsection{UI}
The user interface, in summary, would consist of a power on--off switch, a joystick, a buzzer and an LCD display. The joy stick will be used by the user to control selection on the LCD display. The LCD display will visualize setting the device clock, the dispense schedule and portion sizes and alert the user of warnings. The buzzer is used in order to alert the user or pet when food is being dispensed.
...
\end{table}
\section{Demo}
For the demo we would bring the device and show it by placing it on a table. The next step would be to show how to fill the device with food and set the bowl up to receive the food. We would then plug it into our laptop (or battery), then turn it on. Now that we have done this we can show the setup phase, this will involve turning the device on, and setting up the WIFI. Once we have set the WIFI up we can then set the feeding schedule, for the sake of our demo we would set it to be a time close to the current time. The last setup step is to put the desired amount of food (in weight) into the system, that will be the amount of food dispensed at each time interval. Now we simply wait until the time we set and the feeder should automatically dispense the correct amount of food.
